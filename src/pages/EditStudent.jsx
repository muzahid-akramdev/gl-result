...
import { CheckCircle, AlertCircle, ChevronDown, ChevronUp, Calendar, BookOpen, ArrowLeft } from 'lucide-react'
...
export default function EditStudent() {
  ...
  useEffect(() => {
    supabase.from('students').select('*').eq('id', id).single().then(({ data }) => {
      if (data) {
        setClassNum(data.class_num || '9')
        setExamYear(data.exam_year || new Date().getFullYear().toString())
        setExamType(data.exam_type || EXAM_TYPES[1])
        setForm({ roll: data.roll, name: data.name })
        // load all marks
        const m = {}
        const subs = getSubjects(data.class_num || '9')
        subs.forEach(s => {
          if (s.naib) m[s.naib] = data[s.naib] || ''
          m[s.written] = data[s.written] || ''
        })
        setMarks(m)
      }
      setLoading(false)
  ...
  async function handleSubmit() {
    ...
    setSaving(false)
    if (err) { setError('আপডেট ব্যর্থ: ' + err.message); return }
    setSuccess(true)
    setTimeout(() => navigate(`/marksheet/${id}`), 1000)
  }
  ...
  if (success) return (
    <div className="flex flex-col items-center justify-center h-64 gap-4 animate-scale-in">
      ...
      <p className="bangla text-lg font-semibold text-white">সফলভাবে আপডেট হয়েছে!</p>
    </div>
  )
  ...
  return (
    <div className="px-4 py-5 space-y-4 animate-fade-in">
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        <h2 className="bangla text-lg font-bold text-gold-400">ফলাফল সম্পাদনা</h2>
      </div>
      ...
      <button onClick={handleSubmit} disabled={saving} className="w-full btn-primary py-4 text-base bangla disabled:opacity-50">
        {saving ? 'আপডেট হচ্ছে...' : 'আপডেট করুন ও মার্কশিট দেখুন'}
      </button>
    </div>
  )
}
