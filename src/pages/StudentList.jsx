...
export default function StudentList() {
  ...
  async function downloadBulkPDF() {
    ...
    // keep same order as filtered
    fullData.sort((a, b) => targets.findIndex(s => s.id===a.id) - targets.findIndex(s => s.id===b.id))

    const wrapper = document.createElement('div')
    wrapper.style.cssText = `position:fixed;left:-${MS_WIDTH + 100}px;top:0;width:${MS_WIDTH}px`
    wrapper.innerHTML = fullData.map((s, i) => buildMarksheetHTML(s, { pageBreak: i > 0 })).join('')
